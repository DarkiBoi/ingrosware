...
public class HudComponentCommand extends Command {
    ...
    public void execute(String[] args) {
        if (args.length >= 2) {
            IngrosWare.INSTANCE.getSettingManager().getSettingsFromObject(hudComponent).forEach(setting -> {
                if (args[1].equalsIgnoreCase(setting.getLabel())) {
                    if (setting instanceof BooleanSetting) {
                        final BooleanSetting booleansetting = (BooleanSetting) setting;
                        booleansetting.setValue(!booleansetting.getValue());
                        Logger.printMessage(booleansetting.getLabel() + " has been " + (booleansetting.getValue() ? ChatFormatting.PREFIX_CODE + "aenabled" + ChatFormatting.PREFIX_CODE + "7" : ChatFormatting.PREFIX_CODE + "cdisabled" + ChatFormatting.PREFIX_CODE + "7") + " for " + hudComponent.getLabel() + ".");
                        ...
                        if (args.length >= 3) {
                            if (args[2].equalsIgnoreCase("help")) {
                                ...
                                Logger.printMessage(setting.getLabel() + " has been set to " + setting.getValue() + " for " + hudComponent.getLabel() + ".");
                            }
                        } else {
                        ...
                        if (args.length >= 5) {
                            try {
                                ...
                                Logger.printMessage(setting.getLabel() + " has been set to " + colorSetting.getValue().getRGB() + " for " + hudComponent.getLabel() + ".");
                            } catch (Exception e) {
                                Logger.printMessage("Not enough arguments to change setting.");
                            }
                        } else {
                        ...
                        if (args.length >= 3) {
                            ...
                            if (keyCode != -1) {
                                ...
                                Logger.printMessage(setting.getLabel() + " has been set to " + Keyboard.getKeyName(bindSetting.getValue()) + " for " + hudComponent.getLabel() + ".");
                            } else
